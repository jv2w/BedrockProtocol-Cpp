...
class DynamicValueBool final : public DynamicValue {
public:
    static constexpr std::uint32_t ID = DynamicValueType::BOOL;

    explicit DynamicValueBool(bool value) : value(value) {}

    [[nodiscard]] std::uint32_t getTypeId() const override { return ID; }

    [[nodiscard]] std::unique_ptr<DynamicValue> clone() const override
    {
        return std::make_unique<DynamicValueBool>(*this);
    }

    static DynamicValueBool readValue(encoding::ByteBufferReader &in);
