 ...
 * C++ port of the PHP original: src/types/inventory/stackrequest/PlaceStackRequestAction.php
 ...
 */
class PlaceStackRequestAction final : public ItemStackRequestAction {
public:
    static constexpr std::int32_t ID = ItemStackRequestActionType::PLACE;

    PlaceStackRequestAction(std::uint8_t count, ItemStackRequestSlotInfo source, ItemStackRequestSlotInfo destination) :
    ...
    static PlaceStackRequestAction read(encoding::ByteBufferReader &in) {
        ...
        return PlaceStackRequestAction(count, std::move(src), std::move(dst));
    }
    ...
    [[nodiscard]] std::unique_ptr<ItemStackRequestAction> clone() const override {
        return std::make_unique<PlaceStackRequestAction>(*this);
    }
