...
class StringTag final : public ImmutableTag {
public:
    /**
     * @throws InvalidTagValueException if the string is longer than 32767 bytes
     */
    explicit StringTag(std::string value);

    [[nodiscard]] const std::string &getValue() const { return value; }

    [[nodiscard]] std::int32_t getType() const override;

    static StringTag read(NbtStreamReader &reader);
